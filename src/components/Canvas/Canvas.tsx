    ...
    if (action === 'DRAW') {
      const shape = parts[1]?.toUpperCase() as shapeAction;
			...
			switch (shape) {
				...
				case 'CIRCLE': {
					if (parts.slice(2).length === 0) {
						alert('Нужно добавить x, y, radius и цвет по желанию. Например, draw circle 100 100 10 red')
					}
			...
			if (parts[1]?.toUpperCase() === 'AREA') {
				...
				ctx.clearRect(parseInt(x), parseInt(y), parseInt(width), parseInt(height));
				status = 1;
				return
			}
			status = 1;
